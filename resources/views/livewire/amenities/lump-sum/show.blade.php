 <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
            ...
            <div class="card">
              ...
              <div class="card-body table-responsive p-0" style="height: 460px;">
                <table class="table table-head-fixed text-nowrap">
                  ...
                  <tbody>
								@forelse ($lumpsums as $item)
                                    <tr>
                                        <td>{{$item->position->name}}</td>									
                                        <td>{{$item->poh->name}}</td>
				                        <td>{{$item->site->name}}</td>						
                                        <td>{{currency_IDR($item->idr)}}</td>
                                        <td>{{currency_IDR($item->idr_staff)}}</td>										
										...
										<td><a type="button" href="{{route('lump-sum.edit',$item->id)}}"  title="Ubah"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp; 
									        <a type="button" wire:click="$emitTo('amenities.lump-sum.delete','deleteLumpSum',{{$item->id}})" title="Hapus"><i class="fa fa-trash"></i></a>
									</td>
                                    </tr>
								@empty
									Data tidak ada!
        ...
        <p>
            Showing {{$lumpsums->firstItem()}} to {{$lumpsums->lastItem()}} out of {{$lumpsums->total()}} items
        </p>
        <p>
				{{$lumpsums->links()}}
              </div>
